...
# library(dplyr)
# 
# kiwi_ar <- read_delim("https://raw.githubusercontent.com/chechoid/dpa-itba/main/PEPA/kiwi_ar.csv", delim = ";")
# 
...
```{r q4, exercise=TRUE, exercise.lines = 20}
# Calcular el primer cuartil
q1 <- quantile(kiwi_ar$edad, 0.25)
...
Todo esto fue una excusa para introducir uno de mis gráficos favoritos: los **boxplots**.
...
En el gráfico podemos apreciar que hay 3 *outliers* por encima de los 54 años aproximadamente.
...
Otra medida que se puede calcular es el **coeficiente de variación**, que es una medida de la variabilidad de una variable según el tamaño de la muestra (ver en [Wikipedia](https://es.wikipedia.org/wiki/Coeficiente_de_variaci%C3%B3n)).
...
Más adelante retomamos el tema, por ahora lo dejamos acá.
...
Realizar un histograma de la variable `sueldo_bruto`. ¿Por qué sale así?
...
Ahora crear un data frame nuevo, llamado `kiwi_limpio` filtrando todo lo que esté por debajo del *percentil 5* y todo lo que esté por encima del *percentil 95*.
...
Los data frames `kiwi_ar` y `kiwi_limpio` están almacenados en memoria. Realizar un boxplot de la columna `sueldo_bruto` para ambos datasets. ¿Qué interpretación hacen de los resultados?
...
Por último, graficar un boxplot usando el data frame `kiwi_limpio` mapeando en el eje `x` el `puesto`, y en el eje `y` el `sueldo_bruto`. ¿Qué interpretación hacen de los resultados?
...
Y con esto, cerramos el capítulo de análisis univariado y abrimos el de análisis de dos muestras 🤘.
